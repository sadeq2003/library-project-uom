        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-9">
                    ...
                    $categories = [
                        'حاسب آلي' => 'fas fa-tv',
                        'إعلام'   => 'fas fa-pencil-alt',
                        'اقتصاد'  => 'fas fa-utensil-spoon',
                        'فنية'    => 'fas fa-palette'
                    ];

                    foreach ($categories as $category => $icon) {
                        // استعلام لجلب الكتب في القسم المحدد، ترتيبها حسب عدد التحميلات (تنازلياً) وتحديد عدد النتائج (على سبيل المثال 3 كتب)
                        $query = "SELECT * FROM books WHERE category = '$category' ORDER BY downloads DESC LIMIT 3";
                        $result = mysqli_query($conn, $query);
                        
                        if (mysqli_num_rows($result) > 0) {
                            echo '
                            <div class="computer mb-5">
                                <h3 class="mb-4"><i class="'.$icon.' mr-2"></i>أكثر الكتب تحميلاً في قسم '.$category.'</h3>
                                <div class="row">';
